...
import { Users, MapPin, Layers, Check, Copy, CheckCircle2, Award } from 'lucide-react'
...
export default function ResultDetailsGrid({
  ...
  return (
    <div className="flex flex-col gap-3 w-full">
      {/* Why this place was picked banner */}
      <div className="flex items-start gap-3 p-3.5 sm:p-4 rounded-xl sm:rounded-2xl border bg-muted/40 backdrop-blur-xs">
        ...
        <div className="space-y-0.5 min-w-0">
          <p className="text-xs sm:text-sm font-semibold text-foreground">
            {type === 'consensus' ? 'Unanimous Agreement' : 'Top Compromise Choice'}
          </p>
          <p className="text-xs text-muted-foreground leading-relaxed">
            {type === 'consensus'
              ? `Every single voter in your group of ${participantCount} selected Go on this venue.`
              : `Scored the highest approval and ratings among all ${totalOptions} options considered.`}
          </p>
        </div>
      </div>

      {/* Grid for session stats & location */}
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-2.5 sm:gap-3 w-full">
        {/* Participants count */}
        <Card className="flex flex-col justify-center p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-card border shadow-2xs gap-0.5">
        ...
        <Card className="col-span-2 sm:col-span-1 flex flex-col justify-center p-3 sm:p-4 rounded-xl sm:rounded-2xl bg-card border shadow-2xs gap-1">
