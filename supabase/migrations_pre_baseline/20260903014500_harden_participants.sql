-- =========================================================
-- PARTICIPANTS SECURITY HARDENING & ROOM MEMBERSHIP
-- =========================================================

-- 1. Remove old permissive policies on participants
...
-- =========================================================
-- 2. ROOM MEMBERSHIP HELPER (Prevents recursive RLS evaluation)
-- =========================================================
...
-- =========================================================
-- 3. PARTICIPANT TABLE GRANTS & PRIVILEGES
-- =========================================================

-- Revoke all table-level write/administrative privileges from non-admin roles
REVOKE ALL ON TABLE public.participants FROM anon, PUBLIC;
REVOKE ALL ON TABLE public.participants FROM authenticated;

-- Grant only required privileges to authenticated users
...
BEGIN
    ...
    IF FOUND THEN
        -- Update display name on rejoining if provided, but DO NOT touch is_host or status
...
-- =========================================================
-- 6. SECURE CREATE ROOM WITH HOST RPC
-- =========================================================
...
BEGIN
    ...
    -- 3. Insert Room (defaults to status = 'lobby')
...
REVOKE EXECUTE ON FUNCTION public.create_room_with_host(text, int, int, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_room_with_host(text, int, int, text, text) TO authenticated;


-- =========================================================
-- 7. SECURE START VOTING RPC
-- =========================================================
...
REVOKE EXECUTE ON FUNCTION public.start_voting(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.start_voting(uuid) TO authenticated;


-- =========================================================
-- 8. SECURE SUBMIT VOTE RPC
-- =========================================================
...
BEGIN
    ...
    -- 11. Mark participant as finished
    IF v_finished THEN
        UPDATE public.participants
        SET status = 'finished'
        WHERE participant_id = p_participant_id
...
REVOKE EXECUTE ON FUNCTION public.submit_vote(uuid, uuid, uuid, text) FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.submit_vote(uuid, uuid, uuid, text) FROM anon;
